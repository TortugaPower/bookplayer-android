...
// (Anthropic keys, GitHub tokens, PEM private keys). Each entry is a pattern and its replacement.
export const REPO_SECRET_SHAPES = [
  // Any sentry.io host, not only the modern `o<org>.ingest[.<region>].sentry.io`: the legacy
  // `https://<32 hex>@sentry.io/<id>` form is still valid and still what older projects carry, and it was passing
  // through unredacted. Redaction is the boundary that catches what the path rules cannot, so it is widened
  // rather than kept precise.
  [/https:\/\/[0-9a-f]{16,}(?::[0-9a-f]+)?@[\w.-]*sentry\.io\/\d+/gi, 'https://[redacted]@sentry.io/[redacted]'],
  ...
  [/\b\d{6,}-[a-z0-9]{20,}\.apps\.googleusercontent\.com\b/g, '[redacted client id]'],
  // RevenueCat and store keys. (Keystore passwords are deliberately not pattern-matched: they live only in a
  // gitignored keystore.properties and in Actions secrets, and no useful pattern exists that would not mangle prose.)
  [/\b(goog|appl|amzn|strp|rcb)_[A-Za-z0-9]{20,}\b/g, '[redacted]'],
];
