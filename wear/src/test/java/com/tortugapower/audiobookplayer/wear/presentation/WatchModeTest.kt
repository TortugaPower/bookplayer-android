...
/**
 * The tier → mode gate is the product decision (PRO = standalone, everything else — including no account —
 * = remote; sign-in is a Settings action, not a wall). Locked down here so a future tier tweak can't
 * silently regress it.
 */
class WatchModeTest {
    ...
    @Test fun nullAccount_isRemoteController() =
        assertEquals(WatchMode.REMOTE_CONTROLLER, watchModeFor(null))
